...
export default function PilotPage() {
  const [form, setForm] = useState({ firstname: '', lastname: '', email: '', company: '', org: '' });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (form.org)     params.append('message', `[PILOT] ${form.org}`);
      params.append('hs_context', JSON.stringify({
        pageUri: 'https://longstrider.ai/pilot',
        pageName: 'LongStrider — Start a Pilot',
      }));
  ...
  return (
    <main style={{ minHeight: '100vh', background: 'var(--color-bg)', fontFamily: 'var(--font-body)', position: 'relative', overflow: 'hidden' }}>
      ...
      {/* ── SUCCESS STATE ── */}
      {status === 'success' && (
        <div style={{ position: 'relative', zIndex: 1, maxWidth: '600px', margin: '0 auto', padding: '140px 48px 100px', textAlign: 'center', animation: 'cReveal 0.9s var(--ease-out) both' }}>
          <p style={{ fontFamily: 'var(--font-display)', fontSize: '52px', fontWeight: 300, fontStyle: 'italic', color: GOLD, marginBottom: '28px', lineHeight: 1.05 }}>
            The pilot starts here.
          </p>
          <p style={{ fontSize: '18px', color: 'var(--color-text-secondary)', lineHeight: 1.75, marginBottom: '16px' }}>
            We've got what we need to get started. Expect to hear from us within
            one business day — we'll come prepared.
          </p>
        ...
        <div style={{ position: 'relative', zIndex: 1, maxWidth: '1000px', margin: '0 auto', padding: '112px 48px 100px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '80px', alignItems: 'stretch' }}>
          ...
          <div style={{ animation: 'cReveal 0.9s var(--ease-out) both', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
            ...
            <h1 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(34px, 3.8vw, 54px)', fontWeight: 300, letterSpacing: '-0.03em', lineHeight: 1.1, color: 'var(--color-text-primary)', marginBottom: '28px' }}>
              ...
              <em style={{ fontStyle: 'italic', color: GOLD }}>that lasts.</em>
            </h1>
            ...
            <div style={{ marginBottom: '0' }}>
              <p style={{ fontSize: '16px', lineHeight: 1.75, color: 'var(--color-text-secondary)' }}>
                A pilot isn't a test. It's the beginning of an intelligence layer
                that compounds from day one. We'll work alongside your team,
                deploy in your environment, and build something that belongs to you
                — permanently.
              </p>
            </div>
            ...
            <p style={{
              ...
              fontStyle: 'italic', fontWeight: 300, lineHeight: 1.25,
            ...
            >
              ...
              <div>
                <label style={{ display: 'block', fontFamily: 'var(--font-ui)', fontSize: '10px', letterSpacing: '0.22em', textTransform: 'uppercase', color: 'var(--color-text-secondary)', marginBottom: '10px' }}>
                  Tell us about your organization — what do you want to solve first?
                </label>
                <textarea
                  rows={4}
                  value={form.org}
                  onChange={e => set('org', e.target.value)}
                  placeholder="Size, current stack, the problem you're trying to solve — the more specific, the better."
                  style={{
              ...
              <button
                ...
                style={{
                  alignSelf: 'center', padding: '13px 44px',
              ...
              >
                {status === 'submitting' ? 'One moment…' : 'Start the Pilot'}
              </button>
